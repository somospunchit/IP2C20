...
void main(){
    ...
    for(int x = 0; x < 200; x++){
        sumaTotal += valores[x];
        promedio = x == 199 ? sumaTotal/(x+1) : 0;
    }
    ...
    //1º bucle
    for(int x = 0; x < 200; x++){
        primerElemento = valores[x];
        // Primero agarro el 1º elemento del array para comprobar si alguno de los otros junto a ese, suman "2020".
        // Agarro 1974 y me fijo contra todos si alguno suma 2020

        //2º bucle
        for(int y = 0; y < 200; y++){
                int suma = valores[y] + primerElemento;
        //printf("Suma: %i\n", suma);
            /*if(suma == 2020){
                // Si el resultado es 2020...
                printf("Eureka! %i\n", primerElemento*valores[y]);
            }else {...}*/

            // Operador ternario '?:'
            //suma == 2020 ? printf("Eureka! %i\n", primerElemento*valores[y]) : suma == 2021 ? "SI" : "NO";
            suma == 2020 ? printf("Eureka! %i\n", primerElemento*valores[y]) : 0;
        }
    }

    // printf("x: %i", x); // no anda porque X vive en el ambito del primer bucle
